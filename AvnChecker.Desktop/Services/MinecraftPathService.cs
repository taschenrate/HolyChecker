...
public sealed class MinecraftPathService
{
    private static readonly Regex UsernameRegex =
        new("--username\\s+(?:\"(?<value>[^\"]+)\"|(?<value>\\S+))", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    public string? GetActivePlayerNick()
    {
        foreach (var process in Process.GetProcesses())
        {
            try
            {
                if (!IsMinecraftRelatedProcess(process.ProcessName))
                {
                    continue;
                }
                ...
                var match = UsernameRegex.Match(commandLine);
                if (!match.Success)
                {
                    continue;
                }

                var nick = match.Groups["value"].Value.Trim();
